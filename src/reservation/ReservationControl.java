...
public class ReservationControl {
	...
	public String Explanation( String facility){
			...
			MySQL mysql = new MySQL();
			try {
				ResultSet rs = mysql.getExplanation(facility);
				while(rs.next()){
	...
	//////予約の確認
	public String Confirm( String facility){
			String res = "";//最終的に結果を入れる変数
			String res2 = "";//予約状況の結果を入れる変数
			MySQL mysql = new MySQL();
			try {
				ResultSet rs = mysql.getYoyaku(facility);
				res = facility + "\n";//施設名を一度表示
				while(rs.next()){
					String date = rs.getString("date");
					String start_time = rs.getString("start_time");
					String end_time = rs.getString("end_time");
					res2 += " 年月日： " + date + "    時間： " + start_time + "--" + end_time + "\n";
					}
				//予約がなかった場合
				if(res2==""){
					res2 ="この施設に予約はありません";
						...
						res = "キャンセルの開始時刻と終了時刻が同じです";
							...
							try {
								...
								while(rs.next()){
									...
									String start = rs.getString("start_time");
									String end = rs.getString("end_time");
									if ( start.compareTo(st)==0 && end.compareTo(et)==0){        //キャンセルの開始時刻＝レコードの開始時刻　AND　キャンセルの終了時刻＝レコードの終了時刻 
										// 重複有りの場合にngをtrueに設定
										ng = true; break;
										}
									}
								...
								if (!ng){    //重なっていない場合
									res ="予約キャンセル時間が違います";
										...
										int rs_int = mysql.setCancel(rdate, st, et,facility); 
										res = "予約がキャンセルされました";
										}
								}catch (Exception e) {
									e.printStackTrace();
